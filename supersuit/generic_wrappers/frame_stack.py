        ...
        def modify_obs_space(self, obs_space):
            if isinstance(obs_space, Box):
                ...
                for key in obs_space.keys():
                    self.old_obs_space[key] = obs_space[key]
                    self.observation_space[key] = stack_obs_space(obs_space[key], stack_size)
                self.old_obs_space = Dict(self.old_obs_space)
                return Dict(self.observation_space)
            else:
