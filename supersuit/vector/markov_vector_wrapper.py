    ...
    def __init__(self, par_env, black_death=False):
        ...
        if isinstance(self.observation_space,Dict):
            ...
            for key in self.observation_space.keys():
                self.obs_buffer[key] = np.empty((self.num_envs,) + self.observation_space[key].shape, dtype=self.observation_space[key].dtype)
            # self.obs_buffer = self.observation_space   
        else:
    ...
    def concat_obs(self, obs_dict):
        ...
        for i, agent in enumerate(self.par_env.possible_agents):
            if self.black_death:
                ...
                self.obs_buffer[i] = obs_dict[agent]
        if isinstance(self.obs_buffer,Dict):
            return copy.deepcopy(self.obs_buffer)
