...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.cf_rewards = np.zeros((self.buffer_size, self.n_envs, 1), dtype=np.float32)
    ...
    def compute_svo_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray, polid: int) -> None:
        """
        Post-processing step: compute the lambda-return (TD(lambda) estimate)
        and GAE(lambda) advantage.
        ...
        for step in reversed(range(self.buffer_size)):
            if step == self.buffer_size - 1:
                ...
                next_non_terminal = 1.0 - self.episode_starts[step + 1]
                next_values = self.values[step + 1]
            delta = self.svo_rewards[step] + self.gamma * next_values * next_non_terminal - self.values[step]
            last_gae_lam = delta + self.gamma * self.gae_lambda * next_non_terminal * last_gae_lam
            self.advantages[step] = last_gae_lam
        # TD(lambda) estimator, see Github PR #375 or "Telescoping in TD(lambda)"
        # in David Silver Lecture 4: https://www.youtube.com/watch?v=PnHCvfgC_ZA
    ...
    def compute_inequity_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray, polid: int) -> None:
        """
        Post-processing step: compute the lambda-return (TD(lambda) estimate)
        and GAE(lambda) advantage.
    ...
    def add_svo_sw(
        ...
        self.all_actions[self.pos] = np.transpose(np.array(all_actions).copy(),(1,0,2))
        self.all_rewards[self.pos] = np.transpose(np.array(all_rewards).copy(),(1,0))
        self.svo_rewards[self.pos] = np.array(svo_rewards).copy()
    ...
    def get_sw_traj(self, batch_size: Optional[int] = None, seq_size: Optional[int] = None) -> Generator[RolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            ...
            self.all_rewards_traj = np.array(self.all_rewards_traj,dtype=object)
            # self.previous_all_last_obs_traj = np.array(self.previous_all_last_obs_traj,dtype=object)
            # self.previous_all_actions_traj = np.array(self.previous_all_actions_traj,dtype=object)
            # self.previous_all_rewards_traj = np.array(self.previous_all_rewards_traj,dtype=object)
            for tensor in _tensor_names:
                self.__dict__[tensor] = self.swap_and_flatten(self.__dict__[tensor])
            self.generator_ready = True
