...
class RewardVectorActorCriticPolicy(ActorCriticPolicy):
    """
    Policy class for Vector input and reward prediction.
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        """
        self._build_mlp_extractor()
        self.reward_features_extractor = self.make_reward_features_extractor()
        latent_dim_pi = self.mlp_extractor.latent_dim_pi
        ...
        if self.ortho_init:
            ...
            module_gains = {
                self.features_extractor: np.sqrt(2),
                self.reward_features_extractor: np.sqrt(2),
                self.mlp_extractor: np.sqrt(2),
                self.action_net: 0.01,
                self.value_net: 1,
                self.reward_net: 1,
    ...
    def extract_reward_features(self, obs: th.Tensor, features_extractor: Optional[BaseFeaturesExtractor] = None) -> th.Tensor:
        """
        Preprocess the observation if needed and extract features.
