...
class TransitionActorCriticPolicy(ActorCriticPolicy):
    """
    Policy class for reward prediction and transition modeling.
...
class CausalInfluenceActorCriticPolicy(ActorCriticPolicy):
    """
    Policy class for reward prediction.
    ...
    def _build_lstm_extractor(self) -> None:
        """
        Create the policy and value networks.
        Part of the layers can be shared.
        """
        self.lstm_extractor =  DualLstmExtractor(
            feature_dim=self.features_dim,
            lstm_hidden_dim=64,
            lstm_num_layers=2,
            activation_fn=self.activation_fn,
            policy_dim=self.action_space.n,
            value_dim=1,
            device= self.device
)
    ...
    def forward(self, obs: th.Tensor, obs_seq: th.Tensor = None, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        Forward pass in all the networks (actor and critic)
        ...
        # policy_e, value_e, policy_m, value_m, self.prev_value_m = self.lstm_extractor(features, self.prev_value_m)
        # Evaluate the values for the given observations
        values = self.value_net(latent_vf)
        distribution = self._get_action_dist_from_latent(latent_pi)
        actions = distribution.get_actions(deterministic=deterministic)
