...
from stable_baselines3.common.policies import ActorCriticCnnPolicy, ActorCriticPolicy, BasePolicy, MultiInputActorCriticPolicy, RewardActorCriticPolicy, TransitionActorCriticPolicy, CausalInfluenceActorCriticPolicy
            ...
            if self.model == 'causal':
                if self.enable_trajs_learning:
                    ...
                    for rollout_data in self.rollout_buffer.get_sw(self.batch_size):
                        ...
                        th.nn.utils.clip_grad_norm_(self.policy.parameters(), self.max_grad_norm)
                        self.policy.optimizer.step()
            elif self.model == 'social_influence':
                for rollout_data in self.rollout_buffer.get_social_influence(self.batch_size):
                        self.timestep += 1
                        social_influence = rollout_data.cf_rewards
                        all_last_obs = rollout_data.all_last_obs
                        all_rewards = rollout_data.all_rewards
                        actions = rollout_data.actions
                        if isinstance(self.action_space, spaces.Discrete):
                        ...
                        # Normalize advantage
                        advantages = rollout_data.advantages
                        advantages_e = rollout_data.advantages_e
                        advantages_m = rollout_data.advantages_m

                        # Normalization does not make sense if mini batchsize == 1, see GH issue #325
                        if self.normalize_advantage and len(advantages) > 1:
                            advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
                            advantages_e = (advantages_e - advantages_e.mean()) / (advantages_e.std() + 1e-8)
                        ...
                        policy_loss_e = advantages_e * ratio
                        policy_loss_2_e = advantages_e * th.clamp(ratio, 1 - clip_range, 1 + clip_range)
                        policy_loss_e = -th.min(policy_loss_e, policy_loss_2_e).mean()

                        policy_loss_m = advantages_m * ratio    
                        policy_loss_2_m = advantages_m * th.clamp(ratio, 1 - clip_range, 1 + clip_range)
                        policy_loss_m = -th.min(policy_loss_m, policy_loss_2_m).mean()
                        ...
                        # Value loss using the TD(gae_lambda) target
                        value_loss = F.mse_loss(rollout_data.returns, values_pred)
                        value_loss_e = F.mse_loss(rollout_data.returns_e, values_pred)
                        value_loss_m = F.mse_loss(rollout_data.returns_m, values_pred)
                        ...
                        loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + policy_loss_e + policy_loss_m + value_loss_e + value_loss_m
