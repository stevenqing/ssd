...
METHODs = ['CF_1', 'CF_2', 'CF_5', 'CF_10']
SCENARIOs = ['Coin_Incentive', 'Coin_4_Incentive', 'LBF_Incentive', 'LBF_4_Incentive']
METHODS_NAME = ['1', '2', '5', '10']
COLORs = ['r', "indianred", "darkred", "salmon"]
COLORs = list(reversed(COLORs[:len(METHODs)]))
color_dict = {k: v for k, v in zip(METHODs, COLORs)}
LINE_STYPLEs = ['solid' for _ in range(20)]
pos_dict = {
    'Coin_Incentive': 141,
    'Coin_4_Incentive': 142,
    'LBF_Incentive': 143,
    'LBF_4_Incentive': 144,
}
...
map_scenario_to_name = {f'Coin_Incentive': fr'Coin_3_Agents', f'Coin_4_Incentive': fr'Coin_4_Agents', f'LBF_Incentive': fr'LBF_3_Agents', f'LBF_4_Incentive': fr'LBF_4_Agents'}
map_method_to_name = {f'CF_{alpha}': fr'$\alpha={alpha}$' for alpha in METHODS_NAME}
...
def draw_each(env_name, data_dict, i, color_list, map_method_to_name, label):
    ...
    for method in sorted_methods_list:
        ...
        plt.plot(timestep / 1e8, exponential_moving_average(r_mean, 0.07), color=color,
                 label=map_method_to_name[method],
                 linewidth=lw, linestyle='solid',
                 )
        plt.fill_between(timestep / 1e8, exponential_moving_average(r_mean - r_std, 0.07), exponential_moving_average(r_mean + r_std, 0.1), alpha=0.1,
                         color=color)
...
print(labels)
fig.text(0.085, 0.5, "Counterfactual Regret",
